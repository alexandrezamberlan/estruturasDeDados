...
Celula *excluirTodasOcorrencias(int valor, Celula *lista) {
    Celula *p;
...
int main() {
    ...
    scanf("%d", &numero);
    listaC = excluirTodasOcorrencias(numero, listaC);
    printf("Lista circular depois da exclusão de todas as ocorrências: %d elementos\n", contar(listaC));
    exibir(listaC);
    return 1;
}
