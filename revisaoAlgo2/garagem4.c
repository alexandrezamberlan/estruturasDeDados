...
void entrarGaragem(char placa[], Vaga *vetor, int n) {
    ...
    for (i = 0; i < n; i++) {
        if (strcmp(vetor[i].placa, " ") == 0) {
            ...
            strcpy(vetor[i].dataEntrada, pegaData());
            printf("Data entrada [mm/dd/aa]: %s\n", vetor[i].dataEntrada);
            
            strcpy(vetor[i].horaEntrada, pegaHora());
            printf("Hora entrada [hh:mm:ss]: %s\n", vetor[i].horaEntrada);
...
void sairGaragem(char placa[], Vaga *vetor, int posicaoPlaca) {
	srand(time(NULL));
    strcpy(vetor[posicaoPlaca].dataEntrada, pegaData());
    printf("Data entrada [mm/dd/aa]: %s\n", vetor[posicaoPlaca].dataEntrada);
            
    strcpy(vetor[posicaoPlaca].horaEntrada, pegaHora());
    printf("Hora entrada [hh:mm:ss]: %s\n", vetor[posicaoPlaca].horaEntrada);
